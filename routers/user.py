        ...
        with get_session() as session:
            ...
            if user is not None:
                ...
                return DeepUser(
                    ...
                    blocked_users = [UserSummary(id = blocked_users.id, name=blocked_users.name) for blocked_users in user.blocked_users]
                )
            raise HTTPException(status_code=404, detail="User not found")
    except HTTPException as e:
        raise e
...
@router.post("/remove_block/{id_req}/{blocked_user_id}")
async def remove_block(id_req:int,blocked_user_id: int, authorization: Annotated[str | None, Header()] = None) -> DeepUser:
    if not validate_uid(authorization, id_req):
        raise HTTPException(status_code=403, detail="not authorized")
    if not id_req:
        raise HTTPException(status_code = 400, detail = "Bad request")
    try:
        with get_session() as session:
            user:User | None = session.exec(select(User).where(User.id == id_req)).first()
            blocked_user:User | None = session.exec(select(User).where(User.id == blocked_user_id)).first()
            if user is not None and blocked_user is not None:
                blocked = user.remove_block(blocked_user)
                created = session.exec(select(Event).where(Event.created_by == user.id)).all()
                session.commit()
                if blocked:
                    return DeepUser(
